...
| SDD-30 (TOML example) | `deploy/examples/supervisors/example-daemon.toml` (top-of-file Keychain-ACL contract block + `[child].command[0]` ACL-bound-binary callout); `internal/supervise/config/example_test.go::TestExamples_GenericTOMLValidates` + `TestExamples_NoOperatorSpecificNames`; `docs/CLEAN-MACHINE.md` §8 enumerates the three canonical Keychain entries (`hush-vault-passphrase`, `hush-discord`, `hush-client`) matching `deploy/install.sh` banner verbatim | done |
...
| SDD-30 (Tailscale ACL doc + example template) | `deploy/examples/supervisors/example-daemon.toml` (`server_url = "http://100.64.0.1:7743/h/example"` — concrete CGNAT literal inside `100.64.0.0/10`, canonical hush port 7743, matches `[network] allowed_cidrs`); `docs/TAILSCALE-ACLS.md` realigned to lead with the constitutional `tag:trusted → tag:sandbox:7743` pair (R-002); `internal/supervise/config/example_test.go::TestExamples_GenericTOMLValidates` proves loader-clean | done |
...
- SDD-30 (operator-facing supervisor template — **done**: `deploy/examples/supervisors/example-daemon.toml` populates every CONFIG-SCHEMA.md §Supervisor-config field as an active example value with an inline `# <purpose>. Required.` / `# <purpose>. Default: <loader-default>.` comment, supplying the full schema surface that AC-10's 15 lifecycle scenarios exercise; validated by `internal/supervise/config/example_test.go::TestExamples_GenericTOMLValidates` (SDD-18 loader round-trip) + `TestExamples_NoOperatorSpecificNames` (FR-007 grep gate); top-of-file comment block cross-links `docs/TAILSCALE-ACLS.md` + `docs/CLEAN-MACHINE.md` + the per-binary Keychain ACL contract (AC-6); SDD-30 also re-verified TAILSCALE-ACLS.md and CLEAN-MACHINE.md against current spec/config/install.sh and applied patch-level alignment (R-002 / R-003))
