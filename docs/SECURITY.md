...
| YubiKey touch cache stores the master seed in the Keychain | Medium (opt-in) | When `[yubikey] cache_touch=true`, a successful touch caches the raw 64-byte master seed in a dedicated per-binary-ACL Keychain item (`hush-vault-yubikey-token`) for up to `cache_touch_ttl` (absolute from the last touch, capped 4h by config validation AND the token codec). For that window a Keychain reader **as the hush binary** (root / a compromised binary / ACL bypass) recovers the seed → decrypts the vault and forges JWTs **without the YubiKey**, defeating the hardware-presence guarantee. The token is bound to `SHA-256(envelope ‖ salt)`, so a re-enroll or `vault rekey` invalidates it, and any decode/bind/expiry failure falls back to a touch (fail-closed). **Off by default** so touch-per-restart is preserved; `yubikey-only`+cache is the weakest posture (warned on every store and hit); `hush serve --no-cache` disables it for one run. See [`docs/DAEMONS.md`](DAEMONS.md) §6. |
...
- Why is the YubiKey touch cache off by default? (It trades the
  hardware-presence guarantee — one touch per `serve` restart — for restart
  resilience. Enabling it lets a Keychain reader *as the hush binary* recover
  the master seed for the TTL window, so the default stays touch-per-restart.)
