...
// path verbatim — this helper is test-fixture-only.
...
// supervisor with the supplied name binds. Linux scheme:
// <XDG_RUNTIME_DIR>/hush-supervise-<name>.sock (fallback to
// <TempDir>/hush-supervise-<name>.sock when XDG_RUNTIME_DIR is unset).
//
...
func SocketPathForSupervisor(name string) string {
	...
	return filepath.Join(root, "hush-supervise-"+name+".sock")
}

// EnumerateSupervisorSockets returns the sorted absolute paths of
// every file in the platform runtime directory matching the
// supervisor-socket naming scheme. Linux scheme:
// <XDG_RUNTIME_DIR>/hush-supervise-*.sock.
//
...
func EnumerateSupervisorSockets() ([]string, error) {
	...
	for _, ent := range entries {
		name := ent.Name()
		if !strings.HasPrefix(name, "hush-supervise-") || !strings.HasSuffix(name, ".sock") {
			continue
		}
