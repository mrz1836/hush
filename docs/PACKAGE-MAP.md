...
| `smoke clean` | `hush smoke clean` | Safely clears isolated smoke/test state. Archives `~/.hush-smoke` and `~/.hush-t278-validation` by default, refuses non-smoke state such as `~/.hush`, and requires `--destroy --confirm 'destroy smoke'` for permanent deletion. |
